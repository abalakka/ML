...
def conv2d(x, W):
	return tf.nn.conv2d(x, W, strides=[1,1,1,1], padding='SAME')   #strides = movement of 'window'

def maxpool2d(x):
	return tf.nn.max_pool(x, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')  #ksize = size of 'window' to find 'max' from



def convolution_neural_network(x):
					 					 					
	weights = {'W_conv1':tf.Variable(tf.random_normal([5,5,1,32])),   #5x5 convulutions, 1 input , 32 features
			   'W_conv2':tf.Variable(tf.random_normal([5,5,32,64])),
			   'W_fc':tf.Variable(tf.random_normal([7*7*64, 1024])),
			   'out':tf.Variable(tf.random_normal([1024, n_classes]))}   

	biases = {'b_conv1':tf.Variable(tf.random_normal([32])),   #5x5 convulutions, 1 input , 32 features
